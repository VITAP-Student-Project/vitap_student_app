...
pub fn parse_weekend_outing(html: String) -> Vec<WeekendOutingRecord> {
    ...
    if let Some(table) = document.select(&table_selector).next() {
        ...
        for row in table.select(&row_selector).skip(1) {
            ...
            if cells.len() >= 11 {
                let (contact_number, parent_contact_number, date_idx, booking_id_idx, status_idx, download_idx) = 
                ...
                let booking_id = if is_weekend_format {
                    ...
                    if !booking_id_text.is_empty() {
                        booking_id_text
                    } else if let Some(download_link) = cells[download_idx].select(&download_selector).next() {
