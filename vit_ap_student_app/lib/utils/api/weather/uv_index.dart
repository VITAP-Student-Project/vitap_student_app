Map<String, String> uvIndexWarning(double uv) {
  if (uv >= 0 && uv < 1) {
    return {
      'description':
          'You can safely enjoy enjoy being outdoors without\nany protection!',
  ...
  if (uv >= 2 && uv < 3) {
    return {
      'description':
          'Take some precaution. Seek shade during midday,\nwear SPF 15+ sunscreen if desired',
  ...
  if (uv >= 6 && uv < 7) {
    return {
      'description':
          'Be sun smart! Seek shade during midday!\nSlip on ashirt, slop on sunscreen and slap on hat!',
  ...
  if (uv >= 8 && uv < 9) {
    return {
      'description':
          'Extra caution needed. Minimize midday sun,\nwear sun protection gear & SPF 50+',
  ...
  if (uv >= 11) {
    return {
      'description':
          'Seek shade! Sunburn in minutes. All precautions\nare crucial, including SPF 30+ reapplied often.',
